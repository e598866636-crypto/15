...
class ChipEngine:
    ...
    # ⚠️ 節流設計說明（2026-07-26 新增）：
    # 實測發現 ChipFlowEngine 逐日抓 T86（三大法人）在同一秒內背靠背發出
    # 30 幾個請求時，TWSE 會直接對「後面幾個」甚至「全部」請求回 403
    # Forbidden；同一份 log 裡把請求間隔拉開到 5~10 秒後，改回正常的
    # 「stat=查無資料」回應（合法的空資料，不是被擋）。這強烈指向 TWSE
    # 對短時間內高頻請求有防爬層級的節流／封鎖，不是我們的查詢參數錯誤。
    # 因應方式：
    #   1. 全部請求改用同一個 requests.Session()：重用 TCP 連線，減少
    #      每次都重新握手的開銷與「像是很多台不同機器在打」的訊號。
    #   2. 每次真正送出請求前，確保跟「上一次送出請求」之間至少間隔
    #      _MIN_REQUEST_INTERVAL 秒（含隨機抖動，避免固定週期本身又變成
    #      另一種容易被辨識的機器人特徵）。
    #   3. 若仍然收到 403，視為「暫時被節流」，等一段更長的冷卻時間後
    #      重試最多 _MAX_RETRIES_ON_403 次；重試後仍是 403，才真正判定
    #      這次請求失敗，交由原本的錯誤處理與呼叫端 continue 邏輯處理
    #      （沿用既有的「這天沒資料就跳過」設計，不會讓整條批次流程中斷）。
    # 這套節流是保守的經驗性防禦，不是 TWSE 官方公布的速率限制數字——
    # 如果之後還是偶發 403，可以把 _MIN_REQUEST_INTERVAL 再調大。
    _MIN_REQUEST_INTERVAL = 0.6  # 秒，兩次請求之間的最小間隔
    _REQUEST_JITTER = 0.4        # 秒，額外隨機抖動上限（實際間隔 = MIN ~ MIN+JITTER）
    _MAX_RETRIES_ON_403 = 2
    _BACKOFF_ON_403 = 4.0        # 秒，收到 403 後冷卻再重試的等待時間（每次重試遞增）
    ...
    def _get_session(cls) -> requests.Session:
        """回傳共用的 requests.Session（重用連線，取代每次呼叫都用 requests.get 各開一條）。"""
    ...
    @classmethod
    def _throttle(cls):
        """確保距離上一次送出請求，至少經過 _MIN_REQUEST_INTERVAL（+隨機抖動）秒。"""
        elapsed = time.monotonic() - cls._last_request_ts
        min_gap = cls._MIN_REQUEST_INTERVAL + random.uniform(0, cls._REQUEST_JITTER)
        if elapsed < min_gap:
            time.sleep(min_gap - elapsed)
        cls._last_request_ts = time.monotonic()

    @classmethod
    def _get_with_retry(cls, url: str, params: dict = None, timeout: int = 10):
        """
        節流 + 403 冷卻重試的統一 GET 包裝。回傳 requests.Response（成功或最終失敗
        皆回傳，呼叫端仍需自行檢查 status_code / raise_for_status），或在網路層級
        的例外沒被攔截時往外拋——維持跟原本 requests.get() 呼叫端一致的例外行為，
        只是內部多了節流與 403 重試。
        """
        session = cls._get_session()
        last_exc = None
        for attempt in range(cls._MAX_RETRIES_ON_403 + 1):
            cls._throttle()
            try:
                resp = session.get(url, params=params, timeout=timeout)
            except Exception as e:
                last_exc = e
                break  # 網路層級例外（DNS/逾時等）不屬於「被節流」，不重試，交回呼叫端處理
            if resp.status_code == 403 and attempt < cls._MAX_RETRIES_ON_403:
                logger.warning(
                    f"收到 403（第 {attempt + 1} 次），可能是 TWSE 短時間高頻請求防爬節流，"
                    f"冷卻 {cls._BACKOFF_ON_403 * (attempt + 1):.1f} 秒後重試..."
                )
                time.sleep(cls._BACKOFF_ON_403 * (attempt + 1))
                continue
            return resp
    ...
    def _fetch_institutional_single_day(date_str: str) -> pd.DataFrame:
        try:
            resp = ChipEngine._get_with_retry(
    ...
    def _fetch_shareholding_distribution_full(use_cache: bool = True, max_age_hours: float = 20) -> pd.DataFrame:
        ...
        try:
            resp = ChipEngine._get_with_retry(ChipEngine.TDCC_SHAREHOLDING_URL, timeout=30)
            resp.raise_for_status()
    ...
    def _fetch_insider_holdings_full(use_cache: bool = True, max_age_hours: float = 20) -> pd.DataFrame:
        ...
        try:
            resp = ChipEngine._get_with_retry(ChipEngine.TWSE_INSIDER_HOLDINGS_URL, timeout=30)
            resp.raise_for_status()
            resp.encoding = "utf-8-sig"
            df = pd.read_csv(io.StringIO(resp.text), dtype=str)
        except Exception as e:
