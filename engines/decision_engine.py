...
class DecisionEngine:
    ...
    **誠實、可落地版本**，不是完整的「多代理 AI 決策系統」。它做的事情
    ...
    def build_consensus(latest: pd.Series, canslim_report: dict = None, breakout_report: dict = None,
                         ...
                         news_report: dict = None, chip_flow_report: dict = None) -> dict:
        """
        ...
        純粹讀取與彙整：
            ...
            news_report      NewsEngine.get_news_with_sentiment() 的回傳值
                             ...
                             運作。
            chip_flow_report ChipFlowEngine.compute_institutional_streaks()（+可選的
                             compute_margin_streaks()/compute_chip_score()）
                             彙整後的單一股票結果（v2.9.14 新增）。⚠️ 這裡只用
                             「三大法人合計是否連買且天數是否達門檻」判斷方向，
                             一樣是1票，不因為連買天數很長就加重——ChipFlowEngine
                             裡的 Chip Score（0~100主觀權重公式）刻意不用來決定
                             票數或方向，只會原樣附帶在回傳的文字說明與
                             chip_flow_annotation 欄位中，並明確標示
                             is_validated=False（未經回測驗證），避免一個
                             沒有證據支撐的複合分數被誤認為跟其他票同等可信。
        """
        votes = {}
        ...
        # 12. 籌碼連續動能（v2.9.14 新增）
        # ⚠️ 只用「三大法人合計是否連買且天數是否達門檻」判斷方向，票數只有1
        # （跟其他維度同等權重），不因為連買天數很長就加重——同一套「一個
        # 維度一票」設計，見 class docstring 對「固定權重加總」的反對理由。
        # ChipFlowEngine.compute_chip_score() 算出來的 0~100 分數刻意不用來
        # 決定這裡的方向或票數，只附帶在文字說明裡並明確標示未經驗證，避免
        # 一個沒有回測支撐的複合分數被誤認為跟其他票同等可信。
        CHIP_FLOW_STREAK_THRESHOLD = 3  # 連買/連賣天數達到這個門檻才計入方向判斷
        if chip_flow_report and chip_flow_report.get('total'):
            _total_streak = chip_flow_report['total']
            _direction = _total_streak.get('direction')
            _days = _total_streak.get('days', 0)
            _chip_score = chip_flow_report.get('chip_score')
            _score_str = f"，Chip Score {_chip_score}（⚠️未驗證）" if _chip_score is not None else ""
            if _direction == 'buy' and _days >= CHIP_FLOW_STREAK_THRESHOLD:
                votes['籌碼連續動能'] = (f'偏多（三大法人連買{_days}天{_score_str}）', 1)
            elif _direction == 'sell' and _days >= CHIP_FLOW_STREAK_THRESHOLD:
                votes['籌碼連續動能'] = (f'偏空（三大法人連賣{_days}天{_score_str}）', -1)
            else:
                votes['籌碼連續動能'] = ('中性（未達連續天數門檻）', 0)
        else:
            votes['籌碼連續動能'] = ('資料不足', 0)
