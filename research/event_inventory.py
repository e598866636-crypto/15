...
def summarize_events(events_csv_path: str = None) -> dict:
    ...
    # ⚠️ dtype={"ticker": str}：ETF 代碼（例如 0050）開頭有 0，讓 pandas 自動
    # 推斷型別會被當數字讀掉前導0，變成 50——不只是這裡的 top10/HHI 顯示會
    # 錯，這份 CSV 之後會被 phase_b_forward_return.py 讀回去查詢股價，被
    # 截斷的代碼會直接讓 yfinance 查詢失敗（詳見該檔案的說明）。
