...
def test_ticker_leading_zero_preserved_in_summary():
    """
    迴歸測試：ETF 代碼（如 0050）開頭有 0，summarize_events() 讀回 CSV 時如果
    沒有強制 ticker 欄位為字串型別，pandas 會把它當數字讀掉前導0（0050 -> 50）。
    這不只是顯示問題——被截斷的代碼後續會被 phase_b_forward_return.py 拿去查
    yfinance，導致查無此股票、整批資料抓取失敗（實測發現的真實 bug，不是
    假設性風險）。
    """
    tmp_dir = tempfile.mkdtemp()
    try:
        events = [{"ticker": "0050", "event_date": "2024-01-01", "box_high": 1, "box_low": 1,
                   "box_days": 20, "box_range_pct": 1.0, "volume_confirmed": True,
                   "regime_at_event": "N/A"} for _ in range(5)]
        events += [{"ticker": "2330", "event_date": "2024-01-01", "box_high": 1, "box_low": 1,
                    "box_days": 20, "box_range_pct": 1.0, "volume_confirmed": True,
                    "regime_at_event": "N/A"} for _ in range(5)]
        path = os.path.join(tmp_dir, "leading_zero_events.csv")
        pd.DataFrame(events).to_csv(path, index=False, encoding="utf-8-sig")

        # 直接重現 summarize_events() 內部讀取的方式，確認代碼沒有被截斷
        df_check = pd.read_csv(path, encoding="utf-8-sig", dtype={"ticker": str})
        assert "0050" in df_check["ticker"].unique(), \
            f"ticker 欄位的前導0被截斷了，實際讀到：{df_check['ticker'].unique()}"
        assert 50 not in df_check["ticker"].unique(), "ticker 不應該被讀成數字"

        summary = ei.summarize_events(events_csv_path=path)
        top10_keys = [str(k) for k in summary["top10_tickers"]] if "top10_tickers" in summary else []
        assert not any(k == "50" for k in top10_keys), \
            f"summarize_events() 的統計結果裡出現被截斷的代碼 '50'：{top10_keys}"
        print("✅ test_ticker_leading_zero_preserved_in_summary 通過")
    finally:
        shutil.rmtree(tmp_dir, ignore_errors=True)
...
def run_all():
    ...
    test_ticker_leading_zero_preserved_in_summary()
