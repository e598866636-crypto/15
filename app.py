...
with st.sidebar:
    ...
    if mode == "🔍 個股深度分析":
        ...
        run_btn = st.button(
            "🚀 啟動 AI 多智能體分析", width="stretch", type="primary",
            disabled=(not ticker),
        )
        use_cache = st.checkbox("🗄️ 使用資料庫快取 (建議開啟)", value=True)
        force_refresh = st.button("🔄 強制重新抓取最新資料", width="stretch", disabled=(not ticker))
        scan_btn = False
        rank_btn = False
        chip_flow_btn = False
        macro_btn = False
        ...
        scan_btn = st.button("📡 啟動全台股掃描", width="stretch", type="primary")
        run_btn = False
        force_refresh = False
        rank_btn = False
        chip_flow_btn = False
        ...
        rank_btn = st.button("🏆 抓取最新排行榜", width="stretch", type="primary")
        run_btn = False
        scan_btn = False
        force_refresh = False
        chip_flow_btn = False
        ...
        st.caption(
            "📊 籌碼戰情室：逐日抓取全市場三大法人買賣超與融資融券餘額，"
            "累積成時間序列，計算「連續買賣天數」（不是單日數字），"
            "並依連續天數排序做成多張 Top N 排行榜。"
        )
        st.caption(
            "⚠️ 借券（證券借貸）尚未支援：TWSE 借券頁面確認資料存在，但機器可讀"
            "查詢端點尚未確認，此排行榜暫時無法產生，不會用假資料填充。"
        )
        st.caption(
            "⚠️ Chip Score 是主觀指定、未經回測驗證的複合分數（is_validated=False），"
            "排行榜只是把同一個分數排序顯示，不代表已驗證有效，僅供參考。"
        )
        cf_lookback = st.slider("回溯天數", min_value=10, max_value=60, value=30, key="cf_lookback")
        cf_top_n = st.slider("每項排行榜顯示前 N 名", min_value=5, max_value=50, value=20, key="cf_top_n")
        cf_use_cache = st.checkbox("🗄️ 使用資料庫快取（同日內建議開啟，避免重複逐日抓取）",
                                    value=True, key="cf_use_cache")
        chip_flow_btn = st.button("📊 計算全市場籌碼連續動能", width="stretch", type="primary")
        run_btn = False
        scan_btn = False
        rank_btn = False
        force_refresh = False
        ...
        macro_btn = st.button("🌍 抓取最新總經數據", width="stretch", type="primary")
        run_btn = False
        scan_btn = False
        rank_btn = False
        force_refresh = False
        ...
        news_center_btn = st.button("📰 更新自選股新聞總覽", width="stretch", type="primary")
        run_btn = False
        scan_btn = False
        rank_btn = False
        macro_btn = False
        ...
        research_btn = st.button("🔬 開始事件盤點 (Phase A)", width="stretch", type="primary")
        run_btn = False
        scan_btn = False
        rank_btn = False
        macro_btn = False
    ...
    if "scan_result_df" in st.session_state:
        ...
        if result_df.empty:
            ...
            if not gate_result["passed"].empty:
                ...
                st.dataframe(gate_result["passed"][gate_show_cols], width="stretch", hide_index=True)
            else:
            ...
            with st.expander(f"🔍 查看被淘汰的 {len(gate_result['rejected'])} 檔標的與淘汰原因"):
                if not gate_result["rejected"].empty:
                    ...
                    st.dataframe(gate_result["rejected"][reject_show_cols], width="stretch", hide_index=True)
            ...
            if a_grade_df.empty:
                ...
                st.dataframe(a_grade_df[show_cols], width="stretch", hide_index=True)
            ...
            if trap_df.empty:
                ...
                st.dataframe(trap_df[show_cols_trap], width="stretch", hide_index=True)
            ...
            if industry_df.empty:
                ...
                if selected_industry:
                    ...
                    st.dataframe(constituents, width="stretch", hide_index=True)
                ...
                if rotation_report and rotation_report.get("status") == "ok":
                    st.dataframe(rotation_report["rotation_table"], width="stretch", hide_index=True)
            ...
            if academy_top_df.empty:
                ...
                st.dataframe(academy_top_df[show_cols_ac], width="stretch", hide_index=True)
            ...
            if consensus_df.empty:
                ...
                st.dataframe(consensus_df[show_cols_cs], width="stretch", hide_index=True)
            ...
            if portfolio_result:
                if portfolio_result["status"] != "ok":
                    ...
                    st.dataframe(portfolio_result["weights_table"], width="stretch", hide_index=True)
                    pf_dcol1, pf_dcol2 = st.columns(2)
                    with pf_dcol1:
                        st.markdown("**產業集中度分布**")
                        st.dataframe(portfolio_result["industry_breakdown"], width="stretch", hide_index=True)
                    with pf_dcol2:
                    ...
                    if _illiquid_excl is not None and not _illiquid_excl.empty:
                        with st.expander(f"🔴 已排除 {len(_illiquid_excl)} 檔極低流動性標的"):
                            st.dataframe(_illiquid_excl, width="stretch", hide_index=True)
                    ...
                    with st.expander("📐 投資組合層級 Beta / VaR 匯總（v2.9.6 新增）"):
                        ...
                        if pf_risk:
                            if pf_risk.get("status") == "ok":
                                ...
                                st.dataframe(pf_risk["per_stock"], width="stretch", hide_index=True)
                    ...
                    if rebalance_result:
                        if rebalance_result.get("status") != "ok":
                            ...
                            st.dataframe(rebalance_result["rebalance_table"], width="stretch", hide_index=True)
...
# 📊 籌碼戰情室模式（v2.9.14 新增）
# ==========================================
# ⚠️ 跟上面「🏆 全市場排行榜」的差異：那個只看「單日」買賣超排序；這裡是
# 逐日累積 lookback_days 天的時間序列，排的是「連續天數」，見
# ChipFlowEngine 檔案開頭「連續性比單日更有參考價值」的設計說明。
if mode == "📊 籌碼戰情室":
    if chip_flow_btn:
        with st.spinner(f"逐日抓取近 {cf_lookback} 個交易日的全市場籌碼快照並計算連續天數中（第一次查詢較慢）..."):
            try:
                st.session_state["chip_flow_result"] = ChipFlowEngine.build_chip_flow_dashboard(
                    lookback_days=cf_lookback, top_n=cf_top_n, use_cache=cf_use_cache,
                )
            except Exception as e:
                st.session_state["chip_flow_result"] = {"status": "unavailable", "message": f"⚠️ 籌碼戰情室計算失敗：{e}"}

    cf_result = st.session_state.get("chip_flow_result")

    if cf_result:
        if cf_result.get("status") != "ok":
            st.warning(cf_result.get("message", "⚠️ 暫時無法取得籌碼戰情室資料。"))
        else:
            st.success(f"📅 資料日期：{cf_result['as_of_date']}（累積 {cf_result['days_used']} 個交易日的連續天數資料，僅涵蓋上市股票）")
            if cf_result.get("lending_status") == "not_implemented":
                st.caption(cf_result.get("lending_message", "⚠️ 借券資料尚未支援。"))
            if cf_result.get("margin_status_message"):
                st.caption(f"⚠️ 融資融券資料：{cf_result['margin_status_message']}")

            cf_tab1, cf_tab2, cf_tab3 = st.tabs(["🏛️ 三大法人連續買賣", "💰 融資融券連續增減", "🏆 綜合 Chip Score"])

            with cf_tab1:
                inst_category = st.selectbox("選擇法人類別", ["三大法人合計", "外資", "投信", "自營商"], key="cf_inst_category")
                inst_rankings = cf_result["institutional_rankings"].get(inst_category, {})
                ic1, ic2 = st.columns(2)
                with ic1:
                    st.markdown(f"#### 🟢 {inst_category} 連買前 N 名")
                    st.dataframe(inst_rankings.get("連買前N名", pd.DataFrame()), width="stretch", hide_index=True)
                with ic2:
                    st.markdown(f"#### 🔴 {inst_category} 連賣前 N 名")
                    st.dataframe(inst_rankings.get("連賣前N名", pd.DataFrame()), width="stretch", hide_index=True)
                st.caption("💡 「連續天數」是從最新交易日往回數、方向一致未中斷的天數；累積張數只是同分時的參考，不是排序主鍵。")

            with cf_tab2:
                margin_rankings = cf_result.get("margin_rankings", {})
                if not margin_rankings:
                    st.info("融資融券資料目前無法取得，此頁暫時無排行榜可顯示。")
                else:
                    mc1, mc2 = st.columns(2)
                    with mc1:
                        st.markdown("#### 🔴 融資連續增加前 N 名（散戶追高／籌碼浮動）")
                        st.dataframe(margin_rankings["融資"].get("連增前N名", pd.DataFrame()), width="stretch", hide_index=True)
                        st.markdown("#### 🟢 融資連續減少前 N 名（籌碼沉澱，也可能是恐慌斷頭，需搭配股價判斷）")
                        st.dataframe(margin_rankings["融資"].get("連減前N名", pd.DataFrame()), width="stretch", hide_index=True)
                    with mc2:
                        st.markdown("#### 🔴 融券連續增加前 N 名（放空力道增強）")
                        st.dataframe(margin_rankings["融券"].get("連增前N名", pd.DataFrame()), width="stretch", hide_index=True)
                        st.markdown("#### 🟢 融券連續減少前 N 名（空方回補）")
                        st.dataframe(margin_rankings["融券"].get("連減前N名", pd.DataFrame()), width="stretch", hide_index=True)

            with cf_tab3:
                st.warning(
                    "⚠️ Chip Score 是主觀指定、未經任何回測驗證的複合分數（is_validated=False），"
                    "權重（外資25／投信35／三大法人合計20／融資連減10／融券回補10）沒有統計證據支撐，"
                    "排行榜只是把同一個分數排序顯示，不代表已驗證有效，僅供參考，不構成投資建議。"
                )
                st.dataframe(cf_result["chip_score_ranking"], width="stretch", hide_index=True)
                st.caption("💡 各分項天數欄位刻意保留展開顯示，不是只給一個總分——方便自行判斷要不要信任、要不要重新加權。")

            st.download_button(
                "⬇️ 下載 Chip Score 排行榜 (CSV)",
                data=cf_result["chip_score_ranking"].to_csv(index=False).encode("utf-8-sig"),
                file_name=f"chip_score_ranking_{cf_result['as_of_date']}.csv",
                mime="text/csv",
                key="dl_chip_score_csv",
            )
    else:
        st.info("👈 請在左側點擊「📊 計算全市場籌碼連續動能」。")
...
if mode == "🔬 事件研究實驗室":
    ...
    if ei_summary is None:
        ...
        try:
            ...
            st.dataframe(events_raw_df, width="stretch", hide_index=True)
            st.download_button(
    ...
    with st.spinner("啟動底層特徵引擎與 Agent 辯論中..."):
        ...
        with st.expander("📜 狀態變化歷史"):
            ...
            if _wl_hist.empty:
                ...
                st.dataframe(_wl_hist, width="stretch", hide_index=True)
        ...
        with st.expander("📔 交易日誌（記錄實際成交，計算真實績效）"):
            ...
            with tj_tab2:
                ...
                if _tj_hist.empty:
                    ...
                    st.dataframe(_tj_hist, width="stretch", hide_index=True)
        ...
        try:
            ...
            # 籌碼連續動能（v2.9.14 新增）：需要使用者已在下方「📊 籌碼連續動能」
            # 展開區按過查詢按鈕，才會有 ticker-scoped 的 chip_flow_report；沒有值
            # 時原樣傳 None，DecisionEngine 會顯示「資料不足」（跟 insider_report／
            # news_report 是同一套「選用、預設不觸發全市場逐日抓取」的設計）。
            _chip_flow_report_for_consensus = _get_ticker_scoped_state("chip_flow_report", ticker)
            if _chip_flow_report_for_consensus and _chip_flow_report_for_consensus.get("status") != "ok":
                _chip_flow_report_for_consensus = None
            ...
            consensus = DecisionEngine.build_consensus(
                ...
                news_report=_news_report_for_consensus, chip_flow_report=_chip_flow_report_for_consensus,
            )
        ...
        with st.expander("🏦 大戶持股／千張大戶分析 — 資料源：TDCC集保戶股權分散表，點擊展開後執行"):
            ...
            if shareholding_report:
                if shareholding_report.get("status") != "ok":
                    ...
                    st.dataframe(shareholding_report["tiers"], width="stretch", hide_index=True)
        ...
        with st.expander("🏛️ 董監事／大股東持股與設質分析 — 資料源：證交所董監持股開放資料，僅支援上市股票"):
            ...
            if insider_report:
                if insider_report.get("status") != "ok":
                    ...
                    st.dataframe(insider_report["detail"], width="stretch", hide_index=True)
                    for f in insider_report["flags"]:
                        st.markdown(f"- {f}")

                    if not insider_report["high_pledge_table"].empty:
                        st.markdown("**⚠️ 個人設質比例超過50%的內部人**")
                        st.dataframe(insider_report["high_pledge_table"], width="stretch", hide_index=True)

        with st.expander("📊 籌碼連續動能（選用）— 需要全市場逐日累積資料，點擊展開後執行"):
            st.caption(
                "⚠️ 這裡直接複用「📊 籌碼戰情室」頁面的全市場計算結果（同一份30天回溯資料，"
                "不會為了單一股票重新對 TWSE 逐日發送請求）。如果還沒在該頁面按過"
                "「計算全市場籌碼連續動能」，這裡會先幫你算一次（第一次查詢較慢）。"
            )
            if st.button("📊 查詢本股票的連續買賣天數（複用/計算全市場籌碼資料）", key="chip_flow_ticker_btn"):
                with st.spinner("取得全市場籌碼連續動能資料中..."):
                    try:
                        _cf = st.session_state.get("chip_flow_result")
                        if not _cf or _cf.get("status") != "ok":
                            _cf = ChipFlowEngine.build_chip_flow_dashboard(lookback_days=30, top_n=20, use_cache=True)
                            st.session_state["chip_flow_result"] = _cf
                        if _cf.get("status") == "ok":
                            _inst = _cf.get("institutional_streaks_raw", {}).get(ticker)
                            _score = _cf.get("chip_score_raw", {}).get(ticker)
                            _set_ticker_scoped_state("chip_flow_report", ticker,
                                                      {"status": "ok", "as_of_date": _cf["as_of_date"],
                                                       **(_inst or {}), "chip_score": _score.get("chip_score") if _score else None,
                                                       "chip_score_breakdown": _score.get("breakdown") if _score else None})
                        else:
                            _set_ticker_scoped_state("chip_flow_report", ticker,
                                                      {"status": "unavailable", "message": _cf.get("message", "⚠️ 籌碼資料暫時無法取得。")})
                    except Exception as e:
                        _set_ticker_scoped_state("chip_flow_report", ticker, {"status": "unavailable", "message": f"⚠️ 查詢時發生錯誤：{e}"})

            _chip_flow_report = _get_ticker_scoped_state("chip_flow_report", ticker)
            if _chip_flow_report:
                if _chip_flow_report.get("status") != "ok":
                    st.warning(_chip_flow_report.get("message", "⚠️ 籌碼連續動能資料暫時無法使用。"))
                elif not _chip_flow_report.get("total"):
                    st.info(f"「{ticker}」近期沒有三大法人買賣超的連續天數資料（可能是資料日內掛零或未上市股票）。")
                else:
                    cf_c1, cf_c2, cf_c3, cf_c4 = st.columns(4)
                    for col, label, key in zip(
                        [cf_c1, cf_c2, cf_c3, cf_c4], ["外資", "投信", "自營商", "三大法人合計"],
                        ["foreign", "trust", "dealer", "total"]
                    ):
                        info = _chip_flow_report.get(key, {})
                        direction_label = {"buy": "連買", "sell": "連賣", "flat": "無連續"}.get(info.get("direction"), "無連續")
                        col.metric(label, f"{direction_label} {info.get('days', 0)}天")
                    if _chip_flow_report.get("chip_score") is not None:
                        st.caption(f"⚠️ Chip Score {_chip_flow_report['chip_score']}（主觀指定、未經回測驗證，僅供參考，不構成投資建議）")
        ...
        with st.expander("📅 季節循環分析 (Seasonality) — 需額外抓取10年歷史資料，點擊展開後執行"):
            ...
            if seasonality_report:
                if seasonality_report.get("status") != "ok":
                    ...
                    st.dataframe(seasonality_report["monthly_table"], width="stretch", hide_index=True)
